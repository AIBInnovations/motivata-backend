...
export const createMembershipPlan = async (req, res) => {
  try {
    ...
    return responseUtil.created(res, 'Membership plan created successfully', { plan });
  } catch (error) {
    ...
    return responseUtil.internalError(res, 'Failed to create membership plan', error.message);
  }
};
...
export const getAllMembershipPlans = async (req, res) => {
  try {
    ...
    console.log('[MEMBERSHIP-PLAN] Fetching plans - page:', page, 'limit:', limit, 'isAdmin:', isAdmin);
    ...
    console.log('[MEMBERSHIP-PLAN] Found', plans.length, 'plans out of', totalCount);
...
export const getMembershipPlanById = async (req, res) => {
  try {
    ...
    return responseUtil.success(res, 'Membership plan fetched successfully', { plan });
  } catch (error) {
    ...
    return responseUtil.internalError(res, 'Failed to fetch membership plan', error.message);
  }
};
...
export const updateMembershipPlan = async (req, res) => {
  try {
    ...
    return responseUtil.success(res, 'Membership plan updated successfully', { plan });
  } catch (error) {
    ...
    return responseUtil.internalError(res, 'Failed to update membership plan', error.message);
  }
};
...
export const deleteMembershipPlan = async (req, res) => {
  try {
    ...
    return responseUtil.internalError(res, 'Failed to delete membership plan', error.message);
  }
};
...
export const restoreMembershipPlan = async (req, res) => {
  try {
    ...
    return responseUtil.success(res, 'Membership plan restored successfully', { plan });
  } catch (error) {
    ...
    return responseUtil.internalError(res, 'Failed to restore membership plan', error.message);
  }
};
...
export const createUserMembershipAdmin = async (req, res) => {
  try {
    ...
    console.log('[USER-MEMBERSHIP] Creating admin membership for phone:', normalizedPhone);

    const plan = await MembershipPlan.findOne({ _id: membershipPlanId, isDeleted: false });
    ...
    if (!canPurchase.canPurchase) {
      console.log('[USER-MEMBERSHIP] Plan cannot be purchased:', canPurchase.reason);
    ...
    console.log('[USER-MEMBERSHIP] Admin membership created successfully:', userMembership._id);

    const populatedMembership = await UserMembership.findById(userMembership._id)
    ...
    console.error('[USER-MEMBERSHIP] Error creating admin membership:', error.message);
    ...
    return responseUtil.internalError(res, 'Failed to create user membership', error.message);
  }
};
...
export const createMembershipPaymentOrder = async (req, res) => {
  try {
    ...
    console.log('[MEMBERSHIP-ORDER] Phone:', normalizedPhone, 'Plan:', membershipPlanId);

    const plan = await MembershipPlan.findOne({ _id: membershipPlanId, isDeleted: false, isActive: true });

    if (!plan) {
      console.log('[MEMBERSHIP-ORDER] Plan not found or inactive:', membershipPlanId);
      return responseUtil.notFound(res, 'Membership plan not found or inactive');
    }

    const canPurchase = plan.canBePurchased();
    if (!canPurchase.canPurchase) {
      console.log('[MEMBERSHIP-ORDER] Plan cannot be purchased:', canPurchase.reason);
      return responseUtil.badRequest(res, canPurchase.reason);
    }
    ...
    console.log('[MEMBERSHIP-ORDER] User membership and payment record created');

    return responseUtil.created(res, 'Membership payment order created successfully', {
      orderId: razorpayOrder.id,
      amount,
      currency: 'INR',
    ...
    return responseUtil.internalError(res, 'Failed to create membership order', error.message);
  }
};
...
export const getAllUserMemberships = async (req, res) => {
  try {
    ...
    console.log('[USER-MEMBERSHIP] Fetching memberships - page:', page, 'limit:', limit);
    ...
    console.log('[USER-MEMBERSHIP] Found', memberships.length, 'memberships out of', totalCount);
    ...
    console.error('[USER-MEMBERSHIP] Error fetching memberships:', error.message);
    return responseUtil.internalError(res, 'Failed to fetch user memberships', error.message);
  }
};
...
export const getMyMemberships = async (req, res) => {
  try {
    ...
    console.log('[USER-MEMBERSHIP] Fetching memberships for phone:', normalizedPhone);

    const memberships = await UserMembership.findByPhone(normalizedPhone, false);
    ...
    console.log('[USER-MEMBERSHIP] Found', enrichedMemberships.length, 'memberships');
...
export const checkMembershipStatus = async (req, res) => {
  try {
    ...
    console.log('[USER-MEMBERSHIP] Checking membership status for phone:', normalizedPhone);

    const activeMembership = await UserMembership.findActiveMembership(normalizedPhone);
    ...
    console.log('[USER-MEMBERSHIP] Active membership found:', activeMembership._id);
...
export const getUserMembershipById = async (req, res) => {
  try {
    ...
    const membership = await UserMembership.findOne({ _id: id, isDeleted: false })
      .populate('membershipPlanId')
      .populate('userId', 'name email phone')
      .populate('createdBy', 'name username')
      .populate('updatedBy', 'name username')
    ...
    console.error('[USER-MEMBERSHIP] Error fetching membership:', error.message);

    if (error.name === 'CastError') {
      return responseUtil.badRequest(res, 'Invalid membership ID format');
    }

    return responseUtil.internalError(res, 'Failed to fetch user membership', error.message);
  }
};
...
export const extendUserMembership = async (req, res) => {
  try {
    ...
    console.log('[USER-MEMBERSHIP] Extending membership:', id, 'by', additionalDays, 'days');
    ...
    if (membership.status === 'REFUNDED' || membership.status === 'CANCELLED') {
      console.log('[USER-MEMBERSHIP] Cannot extend cancelled/refunded membership');
      return responseUtil.badRequest(res, 'Cannot extend cancelled or refunded membership');
    }
    ...
    console.error('[USER-MEMBERSHIP] Error extending membership:', error.message);

    if (error.name === 'CastError') {
      return responseUtil.badRequest(res, 'Invalid membership ID format');
    }

    return responseUtil.internalError(res, 'Failed to extend membership', error.message);
  }
};
...
export const cancelUserMembership = async (req, res) => {
  try {
    ...
    if (membership.status === 'CANCELLED' || membership.status === 'REFUNDED') {
      ...
      return responseUtil.badRequest(res, 'Membership already cancelled or refunded');
    }
    ...
    console.error('[USER-MEMBERSHIP] Error cancelling membership:', error.message);

    if (error.name === 'CastError') {
      return responseUtil.badRequest(res, 'Invalid membership ID format');
    }

    return responseUtil.internalError(res, 'Failed to cancel membership', error.message);
  }
};
...
export const updateMembershipNotes = async (req, res) => {
  try {
    ...
    return responseUtil.internalError(res, 'Failed to update admin notes', error.message);
  }
};
...
export const deleteUserMembership = async (req, res) => {
  try {
    ...
    console.error('[USER-MEMBERSHIP] Error deleting membership:', error.message);

    if (error.name === 'CastError') {
      return responseUtil.badRequest(res, 'Invalid membership ID format');
    }

    return responseUtil.internalError(res, 'Failed to delete user membership', error.message);
  }
};
