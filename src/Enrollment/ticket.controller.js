...
export const generateTicketToken = async (req, res) => {
  try {
    ...
    for (const [phone, ticket] of enrollment.tickets) {
      ...
      const verificationUrl = `${req.protocol}://${req.get('host')}/api/web/tickets/verify?token=${token}`;
    ...
    return responseUtil.internalError(res, 'Failed to generate ticket token', error.message);
  }
};
...
export const generateQRCode = async (req, res) => {
  try {
    ...
    const filename = `ticket-${enrollment.eventId.name.replace(/\s+/g, '-')}-${phone}.png`;
    ...
    return responseUtil.internalError(res, 'Failed to generate QR code', error.message);
  }
};
  ...
  try {
    ...
    try {
      ...
      if (error.name === 'TokenExpiredError') {
        return responseUtil.unauthorized(res, 'Ticket has expired', 'TOKEN_EXPIRED');
      }
      if (error.name === 'JsonWebTokenError') {
        return responseUtil.unauthorized(res, 'Invalid ticket - Token is not legitimate', 'INVALID_TOKEN');
      }
      throw error;
    }
    ...
    if (!phone) {
      return responseUtil.badRequest(res, 'Invalid ticket - phone number missing from token');
    }
    ...
    if (!enrollment) {
      return responseUtil.notFound(res, 'Ticket not found or enrollment not found', 'ENROLLMENT_NOT_FOUND');
    }
    ...
    if (!ticket) {
      return responseUtil.notFound(res, `Ticket for phone ${phone} not found`, 'TICKET_NOT_FOUND');
    }

    if (ticket.status !== 'ACTIVE') {
      return responseUtil.badRequest(res, `Ticket is ${ticket.status.toLowerCase()}`, 'TICKET_NOT_ACTIVE');
    }
    ...
    return responseUtil.success(res, 'Ticket verified successfully - Entry granted', {
      isValid: true,
    ...
    return responseUtil.internalError(res, 'Failed to verify ticket', error.message);
  }
};
...
export const generateMockQRLink = async (req, res) => {
  try {
    ...
    const qrScanLinks = [];
    for (const [phone, ticket] of enrollment.tickets) {
      const qrScanLink = `http://192.168.29.105:5000/api/app/tickets/qr-scan?enrollmentId=${enrollmentId}&userId=${enrollment.userId}&eventId=${enrollment.eventId}&phone=${phone}`;
      qrScanLinks.push({
        phone,
        qrScanLink,
        ticketStatus: ticket.status
    ...
    return responseUtil.success(res, 'Mock QR scan links generated successfully', {
      enrollmentId,
    ...
    return responseUtil.internalError(res, 'Failed to generate mock QR link', error.message);
  }
};
...
export const scanQRCode = async (req, res) => {
  try {
    ...
    if (!enrollmentId || !userId || !eventId || !phone) {
      return responseUtil.badRequest(res, 'enrollmentId, userId, eventId, and phone are required');
    }
    ...
    if (!ticket) {
      return responseUtil.notFound(res, `Ticket for phone number ${phone} not found in this enrollment`);
    }

    // Check ticket status
    if (ticket.status !== 'ACTIVE') {
      return responseUtil.badRequest(res, `Ticket for phone ${phone} is ${ticket.status.toLowerCase()}`);
    }

    // Check if already scanned
    if (ticket.isTicketScanned) {
      return responseUtil.badRequest(res, `Ticket for phone ${phone} has already been scanned at ${ticket.ticketScannedAt}`);
    }
    ...
    // Fetch event details
    const event = await Event.findById(eventId).select('name description imageUrls thumbnail mode city category startDate endDate price compareAtPrice availableSeats isLive');
    ...
    return responseUtil.internalError(res, 'Failed to scan QR code', error.message);
  }
};
